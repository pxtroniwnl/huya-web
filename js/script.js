...
const PROGRESS_STAGES = [
    { pct: 10, label: 'Iniciando conexion...' },
    { pct: 30, label: 'Obteniendo datos de NASA POWER...' },
    { pct: 55, label: 'Procesando con modelo de IA...' },
    { pct: 80, label: 'Generando resultados...' },
...
function startProgress() {
    ...
    function tick() {
        ...
        if (!s) { _progressRaf = null; return; }
        const next = PROGRESS_STAGES[stage + 1];
        const target = next ? next.pct : PROGRESS_MAX;
        if (curPct < target) {
            curPct = Math.min(curPct + 0.5, target);
            setProgress(curPct, s.label);
            requestAnimationFrame(tick);
        } else {
            stage++;
            if (!next) { _progressRaf = null; return; }
            setProgress(next.pct, next.label);
            requestAnimationFrame(tick);
        }
    }
    tick();
...
async function calculateWeather() {
    ...
    showProgress(true);
    setProgress(0, PROGRESS_STAGES[0].label);
    startProgress();
...
async function preloadResultadosJson() {
    try {
        const res = await fetch('/api/resultados', { cache: 'no-cache' });
        if (!res.ok) return;
